...
class CrossPlatformFlowMonitor:
    ...
    def setup_toolbar(self):
        ...
        self.disconnect_btn = ttk.Button(toolbar, text="Disconnect", command=self.disconnect_arduino, state=tk.DISABLED)
        self.disconnect_btn.pack(side=tk.LEFT, padx=5)
        
        # Refresh ports button
        ttk.Button(toolbar, text="Refresh Ports", command=self.refresh_ports).pack(side=tk.LEFT, padx=5)
        
        # Status indicator
        self.status_var = tk.StringVar(value="Disconnected")
        self.status_label = ttk.Label(toolbar, textvariable=self.status_var, foreground="red")
    ...
    def setup_plots(self):
        ...
        self.fig.suptitle('🌊 Real-time Liquid Flow Monitoring', fontsize=14, fontweight='bold')
    ...
    def setup_status_bar(self):
        ...
        ttk.Label(self.status_frame, textvariable=self.data_status).pack(side=tk.LEFT, padx=5)
        
        self.stats_status = tk.StringVar(value="Ready")
        ttk.Label(self.status_frame, textvariable=self.stats_status).pack(side=tk.RIGHT, padx=5)
    ...
    def is_valid_data_line(self, line: str) -> bool:
        ...
        skip_prefixes = ("===", "Time", "CSV", "Arduino", "Initializing", "System", "Starting", "---")
    ...
    def parse_data_line(self, line: str):
        ...
        try:
            ...
            if current_pulses > 0:
                logger.info(f"Flow detected: {flow_rate:.3f} L/min ({current_pulses} pulses)")
    ...
    def start_animation(self):
        ...
        try:
            self.animation = animation.FuncAnimation(
                self.fig, self.animate, interval=PLOT_REFRESH_MS, 
    ...
    def update_status(self, flows, volumes, times):
        ...
        try:
            ...
            data_text = f"Flow: {current_flow:.3f} L/min | Volume: {current_volume:.3f} L"
            ...
            if filename:
                with self.thread_lock:
                    with open(filename, 'w') as f:
                        ...
                        for t, fr, tv in zip(self.timestamps, self.flow_rates, self.total_volumes):
